let numberServers = 0;
let serverGlobal;
let measureNameGlobal;
let atualizationGraph01;
let graph;
let graph02;
let statusGraph = 0;
...
function setDatasInGraph(measureName) {
  ...
  graph02.data.datasets[0].label = par;
...
function updateGraph01(server) {
        ...
        .then(function (resposta) {
          if (resposta.status == 200) {
            resposta.json().then((json) => {
              for (let t = 0; t <= 9; t++) {
                if (i == 1) {
                  cpuRegister.unshift(json[t].valor_registro);
                  let dataUTC = new Date(json[t].momento_registro);
                  let options = {
                    timeZone: "America/Sao_Paulo",
                    hour12: false,
  ...
  graph.data.datasets[0].data = cpuRegister;
  graph.data.datasets[1].data = discRegister;
  graph.data.datasets[2].data = memoryRegister;
  graph.data.labels = registerLabels;
