...
function searchDatasByComponent(server, measureName) {
  ...
  SELECT valor_registro, momento_registro  FROM Eyes_On_Server.Registro  as R
	JOIN Eyes_On_Server.Componente_Servidor as CS
    ON R.fk_componente_servidor = CS.id_componente_servidor
    WHERE CS.fk_servidor = (SELECT id_servidor FROM Eyes_On_Server.Servidor WHERE nome_sAAervidor = "${server}")
...
// function selectSectors(company) {
//   let select = `SELECT local_servidor FROM Eyes_On_Server.Servidor WHERE fk_empresa = (SELECT id_empresa FROM Eyes_On_Server.Empresa WHERE nome_fantasia = N'${company}');`;
//   info("selectSectors", select);
//   return bancoDeDados.executar(select);
// }

// function searchServers(local) {
//   let select = `SELECT nome_servidor FROM Eyes_On_Server.Servidor WHERE local_servidor = N'${local}';`;
//   info("searchServers", select);
//   return bancoDeDados.executar(select);
// }
...
// function searchServerInformation(server) {
//   let select = `SELECT nome_servidor, local_servidor, ipv6_servidor, mac_address, so_servidor FROM Eyes_On_Server.Servidor WHERE nome_servidor = N'${server}';`;
//   info("searchServerInformation", select);
//   return bancoDeDados.executar(select);
// }
...
// 	JOIN Eyes_On_Server.Componente_Servidor AS cs
//     ON  r.fk_componente_servidor = cs.id_componente_servidor
//     AND cs.fk_servidor = (SELECT id_Servidor FROM Eyes_On_Server.Servidor WHERE nome_servidor = N'${server}')
//     AND  fk_componente_medida = ${componentID} ORDER BY r.momento_registro DESC LIMIT 1;
//   `;
//   info("searchDatasInLive", select);
//   return bancoDeDados.executar(select);
...
// 	JOIN Eyes_On_Server.Componente_Servidor as CS
//     ON R.fk_componente_servidor = CS.id_componente_servidor
//     WHERE CS.fk_servidor = (SELECT id_servidor FROM Eyes_On_Server.Servidor WHERE nome_servidor = N'${server}')
//     AND CS.fk_componente_medida = ${componentId} ORDER BY R.momento_registro DESC LIMIT 10;`;
//   info("searchDatasForLineGraph", select);
//   return bancoDeDados.executar(select);
// }
...
//     ON R.fk_componente_servidor = CS.id_componente_servidor
//     WHERE CS.fk_servidor = (SELECT id_servidor FROM Eyes_On_Server.Servidor WHERE nome_servidor = N'${server}')
//     AND CS.fk_componente_medida = (SELECT id_componente_medida FROM Eyes_On_Server.Componente_Medida WHERE nome_componente_medida = N'${measureName}' )
//     ORDER BY R.momento_registro DESC LIMIT 10;
//   `;
//   info("searchDatasByComponent", select);
//   return bancoDeDados.executar(select);
